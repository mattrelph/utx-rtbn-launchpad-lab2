...
int OS_AddPeriodicEventThreads(void(*thread1)(void), uint32_t period1,
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
	ptcbs[0].periodFunction = thread1;
	ptcbs[0].period = period1;		
	if (MaxCounter < 	period1)
	{
		MaxCounter = period1;
	}
	ptcbs[1].periodFunction = thread2;
	ptcbs[1].period = period2;		

	if (MaxCounter < 	period2)
	{
		MaxCounter = period2;
	}	
	++MaxCounter;
...
void Scheduler(void){ // every time slice
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
	...
	Counter = (Counter+1)%MaxCounter; // 0 to Largest thread
	if((Counter%ptcbs[0].period) == 0){ // 1, 11, 21, 31 and 41
		(*ptcbs[0].periodFunction)();
	}
	if((Counter%ptcbs[1].period) == 1){ // 0 and 25
		(*ptcbs[1].periodFunction)();
	}
